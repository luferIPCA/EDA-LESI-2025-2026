...
NodePtr DeleteAll(NodePtr h) {
	...
	while (h) {
		aux = h;
		h = h->next;
		//free(aux->val);	//atenção, caso tenha sido criado com malloc!
		free(aux);
	}
	return h;
}
...
void ShowListRec(NodePtr h) {
	if (h) 
		ShowNode(h);
	if (h && h->next) ShowListRec(h->next);	
}
...
bool Save(NodePtr h, char*file) {
	...
	//ATENÇÃO: está a gravar bem? REVER!!!
	Node *current = h;
	Node *holdNext;
	while (current != NULL) {
